"""
Unified Gemini LLM client for the Media Engine, with native caching and retry support.
"""
...
class LLMClient:
    """
    Native Gemini client with hybrid caching support for the Media Engine.
    ...
    def __init__(
        ...
        """
        Initialize Gemini client for Media Engine.
        ...
        """
        if not api_key:
            raise ValueError("Media Engine API key is required.")
        if not model_name:
        ...
        **kwargs
        ...
        try:
            if cache:
                ...
                logger.debug("MediaEngine: Using explicit cached context")
                ...
                logger.debug("MediaEngine: Using implicit caching (standard request)")
            ...
            logger.error(f"MediaEngine generation failed: {e}")
            raise
    ...
    def stream_invoke(
        ...
        **kwargs
        ...
        try:
            ...
            logger.error(f"MediaEngine streaming failed: {e}")
            raise
